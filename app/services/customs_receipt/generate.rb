module CustomsReceipt
  module Generate
    ...
    def write_mock_receipt
      File.write("foo.html", CustomsReceiptTemplate.new(mock_receiptable).call)
    end

    module_function :write_mock_receipt, :run
    ...
    class CustomsReceiptTemplate < Phlex::HTML
      ...
      def view_template
        ...
        html do
          ...
          body do
            img height: 64, style: { display: "inline-block", float: "left" }, src: HC_LOGO, class: "emoji"
            div style: { display: "inline-block", padding_left: "10px" } do
              b { "Hack Club" }
              br
              plain "212 Battery St, Suite 3 "
              br
            ...
            br
            b { "Shipment ID:" }
            plain " " + @r.order_number
            br
            br
            div style: { display: "inline-block" } do
              b { "Ship to: " }
              pre { @r.recipient_address }
            end
            br
            br
            b { "Contents:" }
            div class: "table" do
              ...
              row do
                cell(:right, grid_column: "span 3") do
                  "Total: #{number_to_currency @r.total_value}"
                end
              end
            end
            if @r.shipping_cost.present?
              info "Shipping cost: #{number_to_currency(@r.shipping_cost)}" do
                plain " via #{@r.carrier}" if @r.carrier.present?
              end
            end
            if @r.tracking_number.present?
              info "Original tracking number: #{@r.tracking_number}"
            end
            if @r.additional_info.present?
              br
              info raw(safe(@r.additional_info))
            end
            info do
